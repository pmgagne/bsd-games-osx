...
for game in $GAMES; do
    ...
    if [ -f "$wrapper_path" ]; then
        # Check it's actually one of our generated wrappers: executable, small,
        # and matches the exact template written by install-cli-links.sh
        # (not just a file that happens to mention our marker string somewhere).
        if [ -x "$wrapper_path" ] \
            && [ "$(wc -l < "$wrapper_path" 2>/dev/null | tr -d ' ')" = "4" ] \
            && grep -q "^# BSD Games CLI wrapper for $game\$" "$wrapper_path" 2>/dev/null \
            && grep -q "^exec \"\./$game\" \"\$@\"\$" "$wrapper_path" 2>/dev/null; then
